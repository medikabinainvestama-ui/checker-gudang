...
# --- INISIALISASI SESSION STATE UNTUK PENGATURAN ---
...
def get_user_photo(username):
    ...
    if os.path.exists(photo_path): return photo_path
    return None
...
if not st.session_state['auth']:
    ...
    # --- SIDEBAR & PENGATURAN USER ---
    with st.sidebar:
        user_photo = get_user_photo(st.session_state['user'])
        if user_photo: 
            st.image(user_photo, width=100)
        st.title(f"👤 {st.session_state['user']}")
        
        # Menu Utama
        opt_menu = ["Pemeriksaan QC", "Dashboard Monitoring", "⚙️ Pengaturan Akun"] if is_admin else ["Pemeriksaan QC", "⚙️ Pengaturan Akun"]
        menu = st.radio("Menu Utama", opt_menu)
    ...
    if menu == "⚙️ Pengaturan Akun":
        ...
        # 1. Ukuran Font
        st.subheader("📏 Ukuran Teks")
        st.session_state['font_size'] = st.slider("Atur ukuran font aplikasi:", 12, 24, st.session_state['font_size'])
        
        st.divider()
        
        # 2. Update Foto Profil
        st.subheader("🖼️ Foto Profil")
        up_foto = st.file_uploader("Upload foto profil (PNG/JPG):", type=['png', 'jpg'])
        if up_foto:
            if not os.path.exists("photos"): os.makedirs("photos")
            with open(f"photos/{st.session_state['user'].lower()}.png", "wb") as f:
                f.write(up_foto.getbuffer())
        ...
        pwd_baru = st.text_input("Masukkan Password Baru:", type="password")
        pwd_konf = st.text_input("Konfirmasi Password Baru:", type="password")
        if st.button("Simpan Password Baru"):
            if pwd_baru == pwd_konf and pwd_baru != "":
                update_password_db(st.session_state['user'], pwd_baru)
                st.success("Password berhasil diubah!")
            else:
                st.error("Konfirmasi password tidak cocok atau kosong.")

    elif menu in ["Pemeriksaan QC", "Dashboard Monitoring"]:
        if os.path.exists("data_so.csv"):
            ...
            if menu == "Pemeriksaan QC":
                ...
                def get_info(row):
                    ...
                    return "Pending QC", "-"
                df_mon[['Status', 'Nama QC']] = df_mon.apply(lambda x: pd.Series(get_info(row)) if 'row' in locals() else pd.Series(get_info(x)), axis=1) # Perbaikan apply
